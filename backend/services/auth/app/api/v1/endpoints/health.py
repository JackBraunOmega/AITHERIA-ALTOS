"""
AITHERIA ALTOS - Auth Service Health Endpoints
=============================================

This module implements health check endpoints for the Auth service, providing
...
async def health_check(
    ...
    """
    Get health status of the Auth service and its dependencies.
    
    This endpoint provides information about the service's operational status,
    including database connectivity, Redis connection, and system metrics.
    ...
    if settings.REDIS_URL:
        ...
        try:
            ...
            redis_details["connected"] = redis_connected
            if not redis_connected:
                redis_status = "unhealthy"
                redis_details["error"] = "Failed to connect to Redis"
                health_data["status"] = "unhealthy"
        except Exception as e:
...
async def readiness_probe(db: AsyncSession = Depends(get_db)) -> Dict[str, str]:
    ...
    try:
        ...
        if settings.REDIS_URL:
            redis_connected = await check_redis_connection()
            if not redis_connected:
                return {"status": "not ready", "reason": "Redis connection failed"}
